...
# 把 2D 图像特征 (B, C, H, W) 或 (B, H, W, C) 展开成多条 1D 序列，以便送进 selective_scan 这种状态空间递推算子
# torch.flatten(x, start_dim, end_dim)   从 start_dim 到 end_dim 的所有维度合并成 一个维度
# 怎么合并？像看书一样 从左到右 从上到下
def cross_scan_fwd(x: torch.Tensor, in_channel_first=True, out_channel_first=True, scans=0):
    if in_channel_first:
        B, C, H, W = x.shape # x(BCHW)
        if scans == 0:
            y = x.new_empty((B, 4, C, H * W))
            y[:, 0, :, :] = x.flatten(2, 3)
            y[:, 1, :, :] = x.transpose(dim0=2, dim1=3).flatten(2, 3)
            y[:, 2:4, :, :] = torch.flip(y[:, 0:2, :, :], dims=[-1]) # y: (B, H*W, 4, C)
        elif scans == 1:
    ...
    return y # y: (B, H*W, 4, C)

# 把这些序列（经过 selective_scan 状态空间建模后）再合并回一个 2D 特征图
# scans == 0（四向扫描）
# 先把 左右方向结果对齐（flip），再把 上下方向结果对齐（transpose），最后加和。
# 等价于：左到右 + 右到左 + 上到下 + 下到上。
...
# scans == 3（旋转扫描）
# 分别对旋转 0°, 90°, 180°, 270° 的结果旋转回原位，再逐点加和。
...
# cross_scan1b1_fwd 的处理逻辑：对 4 个通道单独处理，每个通道可以做不同的变换 更灵活
...
# 反操作 然后stack在一起 # •torch.stack：新增维度并堆叠（如将两个 (3, 4) 在 dim=0 堆叠为 (2, 3, 4)）
...
# 自定义backward 因为自动backward每一步都要保存中间结果，要经过flatten、flip等操作会非常消耗显存，所以作者在此先把梯度张量reshape成BHWC，算是给出了捷径。这提醒了我，功能性剪枝极有可能是多分支导致自动backward难以回传梯度造成的
# 梯度在backward的输入的形状就是forward时输出的形状，梯度在backward最后要变成的形状就是forward中输入的形状
...
# @staticmethod 是 Python 中的一个装饰器，用于定义​​静态方法​​。静态方法属于类，但不需要访问类实例（self）或类本身（cls），类似于一个普通的函数，只是被组织在类的命名空间中

# triton implements ========================================

@triton.jit # 用 @triton.jit 装饰的函数会被 Triton 编译器转换为优化的 PTX 代码（类似 CUDA），直接在 GPU 上执行 .            可以把Triton kernel 理解成 GPU 上的 C++/CUDA 风格低层编程
...
# PyTorch + Triton 的混合接口封装，作用是根据输入环境（CPU/GPU/是否支持 Triton）动态选择最优实现
...
#FLOPs（Floating Point Operations，浮点运算量） 定义：模型在一次 前向传播 中执行的浮点数运算次数。常用范围：卷积层、全连接层、注意力机制里最常统计 衡量模型复杂度
...
# 自定义 FLOPs 计数器 🎯， 专门针对 Mamba 的 selective scan 算子 来估算计算量的
def flops_selective_scan_fn(B=1, L=256, D=768, N=16, with_D=True, with_Z=False, with_complex=False):
    ...
    L → sequence length（时序长度 / token 数 / 图像 patch 序列长度）

    D → hidden dim（通道数 / embedding 维度）

    N → 状态空间维度（SSM 的内部维度）

    with_D → 是否包含 skip connection（D 项）

    with_Z → 是否包含输出门控项 z

    with_complex → 是否启用复数运算（Mamba 论文里提到过，但大部分实现是实数）
...
# 更严格，它会模拟 einsum 的真实运算路径，并计算 FLOPs
...
def selective_scan_flop_jit(inputs, outputs, backend="prefixsum", verbose=True):
    '''
    inputs, outputs → 来自 TorchScript JIT 节点的输入输出（fvcore 统计 FLOPs 时会传进来）
    backend → 选择哪种 FLOPs 估算方法：
    "naive" → 用 flops_selective_scan_ref（准确但慢）
    "prefixsum"（默认）→ 用 flops_selective_scan_fn（近似但快）
    verbose=True → 打印输入信息，调试用    
    '''
    if verbose:
...
# 支持 channel_first 模式 支持分组（groups） 
class Linear(nn.Linear):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs): # 改了层的输出/输入维度 就可以自动适配
...
# PyTorch LayerNorm 要给（BCHW）在最后一维做归一化，所以你需要把 channel 维度移到最后：
...
# 与patchembed不同，这个是把（BCHW）变成（B, 4C, H/2, W/2） 保留原始信息和空间结构
class PatchMerge(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        xs = [
            x[..., 0::2, 0::2], x[..., 1::2, 0::2],  # 0::2 是从0开始每隔2取一个    1::2是从1开始每隔2取一个
            x[..., 0::2, 1::2], x[..., 1::2, 1::2],
...
# 一定对空间维度做归一化的softmax，对两个空间维度展平成一个维度L 在此归一化后又变回HW
class SoftmaxSpatial(nn.Softmax):
    def forward(self, x: torch.Tensor):
        if self.dim == -1: # dim代表 对哪维做归一化 对于BCHW         dim=1 → 每行归一化 dim=0 → 每列归一化
            B, C, H, W = x.shape
...
# FFN 就是 Feed-Forward Network（前馈网络） 信息只从输入到输出单向流动 （CNN也是）
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,channel_first=False):
        ...
        # self.act = act_layer()
        self.act = nn.GELU()
        # print(f"WARNING: Using {act_layer} as Mlp activation.")
...
# Depthwise Conv (x)
#    │
# x_proj + dt_proj → dts, Bs, Cs
#    │
# Selective_scan (SSM computation)
#    │
# 拼接 + flip + sum (多方向)
#    │
...
# v2 更加模块化和灵活，适合做多种 SSM 前向实验和 ablation ：in_proj → conv2d → x_proj / dt_proj → selective_scan → cross_merge → out_proj / z门控
class SS2Dv2:
    def __initv2__(
        self,
        # basic dims ===========
        ...
        **kwargs,    
        ...
        self.d_inner = int(ssm_ratio * d_model) # 192
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if forward_type in ["v0", "v0seq"]:
            ...
            self.__initv2__(**kwargs) # 默认使用SS2Dv2
...
# VSSBlock 本质上是一个 通用的 Transformer 风格残差模块，只是把 SS2D（空间状态序列模块）和 MLP（FFN）组合在一起
...
# 完整的 Vision State Space Model (VSS) 框架，本质上类似 Swin Transformer / ConvMixer 的多阶段设计，但用 SS2D 模块代替了自注意力
# VSSM = 输入图像 → patch embedding → 可选位置编码 → stage 序列 → 分类头
...
# 模型验证函数（validate），用来在验证集上评估分类模型的性能
@torch.no_grad()
...
# 测量模型推理吞吐量（throughput），也就是单位时间内模型能处理多少张图片。
@torch.no_grad()
def throughput(data_loader, model):
    ...
    for idx, (images, _) in enumerate(data_loader):
        ...
        for i in range(50): # 预热 GPU，避免第一次前向计算的初始化开销影响吞吐量测量
            model(images)
        torch.cuda.synchronize() # 确保 GPU 完成所有任务再计时
        print(f"throughput averaged with 30 times") # 测 30 次前向推理的总时间
        tic1 = time.time()
        for i in range(30):
            model(images)
        torch.cuda.synchronize()
